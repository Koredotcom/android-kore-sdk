...
class ChatHeaderOneFragment(private val onActionEvent: (event: UserActionEvent) -> Unit) : Fragment() {
        ...
        if (bundle != null) {
            ...
            if (bundle.icon != null) {
                if (bundle.icon?.type == (BotResponseConstants.CUSTOM)) {
                    ...
                    Glide.with(requireActivity()).load(bundle.icon?.iconUrl).apply(RequestOptions.bitmapTransform(CircleCrop()))
