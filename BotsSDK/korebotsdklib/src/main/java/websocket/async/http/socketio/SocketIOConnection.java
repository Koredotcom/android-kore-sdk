...
import codebutler.android_websockets.WebSocketClient.Listener;
...
class SocketIOConnection {
    ...
    private CustomCallback _customCallback = null;
                ...
                try {
                    ...
                    SocketIOConnection.this.webSocketClient =  new WebSocketClient(URI.create(sessionUrl), new Listener() {
                        ...
                        public void onMessage(byte[] data) {
                            //Do nothing
//                        	System.out.println("The res is"+new String(data));
                        ...
                        public void onMessage(String message) {
                            try {
//                                System.out.println("Message is :"+message);// Log.d(TAG, "Message: " + message);
                                ...
                                case 5: {
                                	HashMap<String, String> _hash = new HashMap();
                                    final String dataString = parts[3];
                                    final JSONObject data = new JSONObject(dataString);
                                    final String event = data.getString("name");
                                    if(event != null){
                                    	_hash.put("name", event);
                                    }
                                    final JSONArray args = data.optJSONArray("args");
                                    
//                                    final JSONArray keyArray = input.getJSONArray("key");
//                                    String[] keyAttributes = new String[args.length()];
//                                    for(int i = 0; i < args.length(); i++) {
//                                        keyAttributes[i] = args.getString(i);
//                                    }
                                    if(args != null){
                                    	_hash.put("args", args.toString(0));
                                    }
//                                    System.out.println("##### " + args.toString(0));
//                                    HashMap<String , Object> hmap= (HashMap<String, Object>) JsonHelper.toMap(data);
                                    reportEvent(parts[2], event, args, acknowledge(parts[1]));
                                    if(_customCallback != null)_customCallback.onStringRecived(_hash);
                                    break;
                                }
                                case 6:
                                    // ACK
   ...
   /* private void select(String endpoint, SelectCallback callback) {
        for (SocketIOClient client : clients) {
            if (endpoint == null || TextUtils.equals(client.endpoint, endpoint)) {
                callback.onSelect(client);//p
            }
        }
    }*/
    //Suggested by Girish and Tirupathi to avoid concurrent modification exception
    private void select(String endpoint, SelectCallback callback) {
        ArrayList<SocketIOClient> copyClients = new ArrayList<>(clients);
        for (SocketIOClient client : copyClients) {
            if (endpoint == null || TextUtils.equals(client.endpoint, endpoint)) {
                callback.onSelect(client);//p
            }
        }
