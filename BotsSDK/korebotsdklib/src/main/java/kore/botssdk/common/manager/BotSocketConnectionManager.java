...
import static kore.botssdk.common.manager.BaseSocketConnectionManager.CONNECTION_STATE.CONNECTING;
import static kore.botssdk.common.manager.BaseSocketConnectionManager.CONNECTION_STATE.DISCONNECTED;
...
import kore.botssdk.common.utils.NetworkUtility;
...
//        KoreEventCenter.post(new SocketDataTransferModel(BaseSocketConnectionManager.EVENT_TYPE.TYPE_TEXT_MESSAGE,payload,null));
    ...
    public void persistBotMessage(String payload, boolean isSentMessage, BotRequest sentMsg) {
        ...
        new BotDataPersister(context, userId, payload, isSentMessage, sentMsg)
                .loadDataFromNetwork()
...
//                }
    ...
    @Override
    public void startAndInitiateConnectionWithAuthToken(Context context, String userId, String accessToken, RestResponse.BotCustomData botCustomData) {
        if (connection_state == null || connection_state == DISCONNECTED) {
            ...
            ttsSynthesizer = new TTSSynthesizer(context);
//            this.socketUpdateListener = socketUpdateListener;
    ...
    @Override
    public void startAndInitiateConnectionWithConfig(Context context, RestResponse.BotCustomData botCustomData1) {
...
//            this.socketUpdateListener = socketUpdateListener;
            /*if (!botsSpiceManager.isStarted())
                botsSpiceManager.start(this.context);*/
            initiateConnection();
        }
    }

    @Override
    public void startAndInitiateConnection(Context context, String userId, String accessToken, UserNameModel userNameModel, String orgId) {

    }


    private void initiateConnection() {
        if (!NetworkUtility.Companion.isNetworkConnectionAvailable(context)) {
//            Toast.makeText(context, "No Network", Toast.LENGTH_SHORT).show();
...
//        KoreEventCenter.post(new SocketDataTransferModel(BaseSocketConnectionManager.EVENT_TYPE.TYPE_MESSAGE_UPDATE,message,botRequest));
    ...
    public void checkConnectionAndRetry(Context context, boolean isFirstTime) {
    ...
    public void checkConnectionAndRetryForSignify(Context context, boolean isFirstTime) {
        ...
        if (botClient == null) {
            ...
            if (botCustomData == null) {
                ...
                SharedPreferences preferences = context.getSharedPreferences("signify_preferences", MODE_PRIVATE);
