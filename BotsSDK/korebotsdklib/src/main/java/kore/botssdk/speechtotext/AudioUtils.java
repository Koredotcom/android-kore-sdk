...
public class AudioUtils {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (!info.getName().startsWith("OMX.")) {
                    ...
                    // using MediaCodec.
                    Log.i("AudioUtils","skipping '" + info.getName() + "'.");
                    continue;
                }
    ...
    public static MediaCodec createCodec(String componentName, MediaFormat format) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            try {
                ...
                Log.e("AudioUtils","codec '" + componentName + "' failed configuration.");
            } catch (IOException e) {
                Log.e("AudioUtils","codec '" + componentName + "' failed configuration.");
            }
        }
    ...
    public static void showMetrics(MediaFormat format, int numBytesSubmitted, int numBytesDequeued) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            Log.i("AudioUtils","queued a total of " + numBytesSubmitted + " bytes, " + "dequeued " + numBytesDequeued + " bytes.");
            ...
            Log.i("AudioUtils","desiredRatio = " + desiredRatio + ", actualRatio = " + actualRatio);
        }
    }
    ...
    public static void showSomeBytes(String tag, byte[] bytes) {
        Log.i("AudioUtils","enc: " + tag + ": length: " + bytes.length);
        String str = "";
        ...
        if (len > 0) {
            ...
            Log.i("AudioUtils","enc: " + tag + ": hex: " + str);
        }
    }
}
