    ...
    private void onpacket(Packet<?> packet) {
        ...
        switch (packet.type) {
            ...
            case Parser.EVENT: {
                ...
                Log.d("IKIDO",p.data.toString());
                HashMap<String, String> _hash = new HashMap();
                try {
                    final JSONArray data = new JSONArray(p.data.toString());
                    if (data != null && data.length()>1) {
                        if(data.get(0) instanceof String && data.get(1) instanceof JSONObject){
                            String name = data.get(0).toString();
                            JSONObject obj = data.getJSONObject(1);
                            _hash.put("name", name);
                            _hash.put("args", obj.toString(0));
                            if (this.connected) {
                                super.emit("kora", _hash);
                            }
                        }
                    }
                }
                    ...
                    e.printStackTrace();
                    Log.d("IKIDO","Exception " + ":" + e.toString());
                }
                this.onevent(p);
                break;
            }
