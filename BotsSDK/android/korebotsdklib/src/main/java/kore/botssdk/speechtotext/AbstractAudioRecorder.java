...
public abstract class AbstractAudioRecorder implements AudioRecorder {
    ...
    //Listener to send data to server
...
//        mRecording = new byte[mOneSec * 60];
    }


    protected void clearBuffer(){
        mRecording = null;
        mRecording = new byte[mOneSec * 300];
    }
    ...
    protected int getBufferSize() {
        ...
        if (minBufferSizeInBytes == SpeechRecord.ERROR_BAD_VALUE) {
            ...
            Log.e("AbstractAudioRecorder","SpeechRecord.getMinBufferSize: unable to query hardware for output properties");
            minBufferSizeInBytes = mSampleRate * (120 / 1000) * RESOLUTION_IN_BYTES * CHANNELS;
        }
        int bufferSize = 8000;//BUFFER_SIZE_MUTLIPLIER * minBufferSizeInBytes;
        Log.i("AbstractAudioRecorder","SpeechRecord buffer size: " + bufferSize + ", min size = " + minBufferSizeInBytes);
        return bufferSize;
    }
    ...
    protected int getStatus(int numOfBytes, int len) {
        Log.i("AbstractAudioRecorder","Read bytes: request/actual: " + len + "/" + numOfBytes);
        if (numOfBytes < 0) {
            Log.e("AbstractAudioRecorder","AudioRecord error: " + numOfBytes);
            return numOfBytes;
        }
        if (numOfBytes > len) {
            Log.e("AbstractAudioRecorder","Read more bytes than is buffer length:" + numOfBytes + ": " + len);
            return -100;
        } else if (numOfBytes == 0) {
            Log.e("AbstractAudioRecorder","Read zero bytes");
            return -200;
        } else if (mRecording.length < mRecordedLength + numOfBytes) {
            Log.e("AbstractAudioRecorder","Recorder buffer overflow: " + mRecordedLength);
            return -300;
        }
        return 0;
    }
    ...
    protected int read(SpeechRecord recorder, byte[] buffer) {
        ...
        if (status == 0) {
            ...
            if(audioDataReceivedListener != null && getRmsdb() > 20.0) {
                audioDataReceivedListener.onAudioDataToServer(buffer);
//                Log.d("Abstract","The sound is "+getRmsdb());
            }
            mRecordedLength += len;
        }
        return status;
    ...
    public AudioDataReceivedListener getAudioDataReceivedListener() {
        return audioDataReceivedListener;
    }
    ...
    protected byte[] getCurrentRecording(int startPos) {
        ...
        Log.i("AbstractAudioRecorder","Copied from: " + startPos + ": " + bytes.length + " bytes");
        return bytes;
    }
    ...
    public boolean isPausing() {
        ...
        Log.i("AbstractAudioRecorder","Pause score: " + pauseScore);
        return pauseScore > 7;
    }
        ...
        if (getSpeechRecordState() == SpeechRecord.STATE_INITIALIZED) {
            ...
            if (mRecorder.getRecordingState() == SpeechRecord.RECORDSTATE_RECORDING) {
                ...
                if(audioDataReceivedListener != null)
                    audioDataReceivedListener.onRecordingStarted();
                new Thread() {
                    public void run() {
    ...
    public void stop() {
        ...
        if (getSpeechRecordState() == SpeechRecord.STATE_INITIALIZED &&
            ...
            try {
                ...
                if(audioDataReceivedListener != null)
                    audioDataReceivedListener.onRecordingStopped();
    ...
    protected void handleError(String msg) {
        ...
        Log.e("AbstractAudioRecorder",msg);
    }
